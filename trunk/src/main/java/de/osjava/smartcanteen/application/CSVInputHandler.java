 ...
 * werden eine rudimentaere Fehlerbehandlung beinhalten, welche das Einlesen von inkonsistenten Zeilen abfaengt. Die Logik
 * der Zerlegung der eingelesenen Zeilen in die einzelnen Datenfelder stellt die Klassenmethode splitLine() zur Verfuegung
 * 
...
public class CSVInputHandler {
    ...
    public CSVInputHandler(URL filename, char delimiter) throws IOException {
     ...
     * Zeilen mit einer abweichenden Feldgroe§e gemessen zum Header werden uebersprungen
     * 
     * @return Liefert TRUE wenn noch eine Zeile vorhanden und FALSE wenn nicht
     * @throws IOException
     */
    public boolean hasMoreLines() throws IOException {
        // Array Variable vom Typ Array of String der den Inhalt der Felder einer Zeile aufnimmt. Der Inhalt der felder wird in
        ...
        if (nextLine == null)
            ...
            while ((nextLine = reader.readLine()) != null) {
            	...
            	if (nextLine.contains("š".replace("š", "oe"))){
                	
                }
                
                if (nextLine.contains("Š".replace("Š", "ae"))){
                	
                }
                
                if (nextLine.contains("Ÿ".replace("Ÿ", "ue"))){
    ...
    /**
     * 
     Die Methode readCSVFile() uebernimmt als Argumente den Namen der CSV-Datei und das Trennzeichen. In einer einzigen while-Schleife werden die einzelnen Zeilen eingelesen, von Whitespace 
     an den Enden befreit und dann mit Hilfe von split() in Felder aufgeteilt. (Leerzei- len werden zuvor ausgesondert.)
     * 

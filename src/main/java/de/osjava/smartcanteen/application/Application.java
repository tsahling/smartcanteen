...
// Hallo zusammen,
//
// die Eingabedateien stehen jetzt als Download bereit. Die Hitliste enth�lt eine laufende Nummer und das Gericht (d.h.
// Nummer 1 ist das beliebteste Gericht usw.). Die Rezepte enthalten in jeder Zeile Rezeptname, Menge, Einheit, und den
// Namen der Zutat. Bei den Preislisten enth�lt die erste Zeile die Angabe ob es ein Bauer oder Gro�h�ndler ist, den
// Namen des Anbieters, und die Angabe der Entfernung bzw. des Kostensatzes, je nachdem. Danach folgen die einzelnen
// Angebotenen Waren: Menge des Gebindes, Einheit, Name, dann ggf. f f�r Fisch und m f�r Fleisch, Preis des Gebindes,
// und Anzahl vorhandener Gebinde.
//
// Die Namen der Dateien soll man �bergeben k�nnen, damit ich ihr Programm auch dann verwenden kann, wenn es z.B. mehr
			...
			if (inputFileSplit.length > 0) {
				for (String inputFile : inputFileSplit) {
					...
					if (inputFileUrl != null) {
						...
						// Dateisystem in die Verwaltungsklassen
					} else {
						wrongInputFile = true;
						break;
					}
